...
(function (module, __dirname) {
    ...
    function compileSite(cache, mod, url, debug, cb) {
    ...
    //            cb();
                return compileHtml(cache, mod, url.pathname, undefined, debug, cb);
            });
        } else {
//            cb();
            return compileHtml(cache, mod, url.pathname, undefined, debug, cb);
...
//            TODO: this is not cached.
            ...
            return compileSite(cache, mod, parsed, debug, function (err, mod2, compiled, statusCode) {
                if (err) {
                    return next(err);
                }
